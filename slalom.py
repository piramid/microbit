...
while True:
    ...
    i = m.Image('00000:'*5) # had 5 row LED
    s = i.set_pixel   # set brightness of pixel
    ...
    while True:
        ...
        if not (player_update or wall_update):
            next_event = min(wall_next, player_next) # most min value to next_event
            delta = next_event - t
            m.sleep(delta)
            continue

        if wall_update: # if = wall_update
